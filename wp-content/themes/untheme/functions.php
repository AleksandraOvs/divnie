...
function untheme_setup()
{
	...
	register_nav_menus(
		array(
			'main_menu' => esc_html__('Primary', 'untheme'),
			'services' => esc_html__('Services', 'untheme'),
			'footer_menu' => esc_html__('Footer menu', 'untheme'),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'untheme_custom_background_args',
			array(
...
function untheme_widgets_init()
{
	register_sidebar(
		array(
			'name'          => esc_html__('Footer-widget #1', 'untheme'),
			'id'            => 'footer-sidebar1',
...
function untheme_scripts()
{
	...
	wp_enqueue_style('untheme_styles', get_stylesheet_directory_uri() . '/css/style.css', array(), time());
